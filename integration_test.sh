...
# Enable verification of base images.
export COSIGN_EXPERIMENTAL=true
export KOFLAGS="--local --strict"
...
RESULT="$(GO111MODULE=off ./ko publish ${KOFLAGS} github.com/go-training/helloworld  | grep "$FILTER" | xargs -I% docker run %)"
...
pushd .. || exit 1
RESULT="$(GO111MODULE=auto GOFLAGS="-mod=vendor" ./ko/ko publish ${KOFLAGS} github.com/go-training/helloworld  | grep "$FILTER" | xargs -I% docker run %)"
if [[ "$RESULT" != *"Hello World"* ]]; then
  echo "Test FAILED. Saw $RESULT" && exit 1
else
  echo "Test PASSED"
...
RESULT="$(GO111MODULE=auto GOFLAGS="-mod=vendor" ./ko publish ${KOFLAGS} github.com/go-training/helloworld  | grep "$FILTER" | xargs -I% docker run %)"
if [[ "$RESULT" != *"TEST"* ]]; then
  echo "Test FAILED. Saw $RESULT" && exit 1
else
  echo "Test PASSED"
...
RESULT="$(GO111MODULE=auto GOFLAGS="" ./ko publish ${KOFLAGS} github.com/go-training/helloworld  | grep "$FILTER" | xargs -I% docker run %)"
if [[ "$RESULT" != *"TEST"* ]]; then
  echo "Test FAILED. Saw $RESULT" && exit 1
else
  echo "Test PASSED"
...
RESULT="$(GO111MODULE=on GOFLAGS="-mod=vendor" ./ko publish ${KOFLAGS} github.com/go-training/helloworld  | grep "$FILTER" | xargs -I% docker run %)"
if [[ "$RESULT" != *"TEST"* ]]; then
  echo "Test FAILED. Saw $RESULT" && exit 1
else
  echo "Test PASSED"
...
RESULT="$(GO111MODULE=on GOFLAGS="" ./ko publish ${KOFLAGS} github.com/go-training/helloworld  | grep "$FILTER" | xargs -I% docker run %)"
if [[ "$RESULT" != *"TEST"* ]]; then
  echo "Test FAILED. Saw $RESULT" && exit 1
else
  echo "Test PASSED"
...
pushd .. || exit 1
GO111MODULE=on ./ko/ko publish ${KOFLAGS} github.com/go-training/helloworld && exit 1
